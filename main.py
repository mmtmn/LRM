...
class LRMv2(nn.Module):
    ...
    def forward(self, seq):
        device = self.entity_mem.memory.device
        seq_vecs = []
        for h, r, t in seq:
            h_vec = self.entity_mem.memory[h]
            r_vec = self.relation_emb(torch.tensor(r, device=device))
...
# ---------- Training ----------
def train_lrm(dataset_dir="data/FB15k-237/data", epochs=20, emb_dim=128):
    ...
    for epoch in range(epochs):
        ...
        for seq, next_rel_str in dataset:
            opt.zero_grad()
            pred = model(seq)
            next_rel = rel2id[next_rel_str]
...
if __name__ == "__main__":
    model, ent2id, rel2id = train_lrm("data/FB15k-237/data", epochs=20)
